...
public class CarrelloServlet extends HttpServlet {
    ...
    private void handleShowAction(final HttpSession session, final ProdottoDAO prodottoDAO, final PrintWriter out)
            throws IOException {
    ...
    // metodo che viene usato quando viene modificata la quantita di un prodotto nel
    // carrello
    public void handleQuantityVariantAction(final HttpServletRequest request, final HttpSession session,
            final ProdottoDAO prodottoDAO, final PrintWriter out) throws IOException {
        ...
        if (p != null) {
            ...
            final Variante v = varianteDAO.doRetrieveVariantByFlavourAndWeight(p.getIdProdotto(), gusto, pesoConfezione)
                    .get(0);
            final List<Carrello> cartItems = (List<Carrello>) session.getAttribute("cart");
            if (v != null && cartItems != null) {
                final String quantity = request.getParameter("quantity");
                // controlla che la quantita passata sia corretta e che sia inferiore alla
                // quantita presente nel DB
    ...
    public void handleRemoveVariantAction(final HttpServletRequest request, final HttpSession session,
            final ProdottoDAO prodottoDAO, final PrintWriter out) throws IOException {
    ...
    private void handleAddVariantAction(final HttpServletRequest request, final HttpSession session,
            final ProdottoDAO prodottoDAO, final PrintWriter out) throws IOException {
        // prendo il prodotto
        ...
        if (p != null) {
            ...
            final Variante v = varianteDAO
                    .doRetrieveVariantByFlavourAndWeight(id, gusto, Integer.parseInt(pesoConfezione)).get(0);
    ...
    // metodo che per ogni prodotto nel carrello crea un oggetto JSON e lo inserisce
    // in un JSONArray
    // metodo che crea quindi il carrello effettivo
    private void writeCartItemsToResponse(final List<Carrello> cartItems, final ProdottoDAO prodottoDAO,
